...
const Tasks = () => {
  ...
  useEffect(() => {
    (async () => {
      const resp = await fetch(
        `${process.env.NEXT_PUBLIC_BACKEND_BASE_URL}/tasks/all`
      );
  ...
  async function submit() {
    ...
    const resp = await fetch(
      `${process.env.NEXT_PUBLIC_BACKEND_BASE_URL}/tasks/add`,
      {
        method: "POST",
        headers: {
        ...
        <div className="overflow-x-auto rounded-lg shadow">
          <table className="min-w-full divide-y divide-gray-200 bg-white">
            <thead className="bg-green-600">
              <tr>
                <th className="px-6 py-3 text-left text-lg font-semibold text-white w-[15%]">
                  Task ID
                </th>
                <th className="px-6 py-3 text-left text-lg font-semibold text-white w-[15%]">
                  Employee Name
